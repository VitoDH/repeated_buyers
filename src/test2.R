...
setwd("E:/文档/tmp/IJCAI15 Data/data_format1")
...
#提取用户特征，根据用户特征进行聚类，给出两个属性，一为商标聚类，二为商品种类聚类
BuildFeatVec <- function(tempList){        #------------用于对clusterUser和clusterSeller的lapply,对整个List loop一遍非常慢！
  #age_range <- as.numeric(tempList[1,8])  如果要聚类就要用到
  #gender <- as.numeric(tempList[1,9])
...
write.csv(FinalMat,"trainSet2.csv")
